...
export const log = {
  info: (tag: string, msg: string, data?: Record<string, unknown>): void => {
    const line = data
      ? `[ADMIN] ${ts()} INFO  [${tag}] ${msg} ${JSON.stringify(data)}`
      : `[ADMIN] ${ts()} INFO  [${tag}] ${msg}`;
    console.log(line);
  },
  warn: (tag: string, msg: string, data?: Record<string, unknown>): void => {
    const line = data
      ? `[ADMIN] ${ts()} WARN  [${tag}] ${msg} ${JSON.stringify(data)}`
      : `[ADMIN] ${ts()} WARN  [${tag}] ${msg}`;
    console.warn(line);
  },
  error: (tag: string, msg: string, err?: unknown, data?: Record<string, unknown>): void => {
    ...
    const line = data
      ? `[ADMIN] ${ts()} ERROR [${tag}] ${msg} error=${errStr} ${JSON.stringify(data)}`
      : `[ADMIN] ${ts()} ERROR [${tag}] ${msg} error=${errStr}`;
    console.error(line);
  },
};
